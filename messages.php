	...
	<ul>
		<li style="float: left;">
			...
			<div>
				...
				$grab_messages = mysqli_query("SELECT * FROM pvt_messages WHERE user_to='$user' OR user_from='$user' ORDER BY id DESC");
				...
				while ($get_msg = mysqli_fetch_assoc($grab_messages)) {
					...
					$get_userto_info = mysqli_query("SELECT * FROM users WHERE username='$user_to'");
						...
						if (mysqli_num_rows($check)===1) {
							...
							if ($username != $user) {
								if (isset($_POST['submit'])) { 
									...
									$messages = mysqli_query("INSERT INTO pvt_messages VALUES ('','$user','$username','$msg_body','$date','','$opened','')");
